...
function CriteriaTable({ bucket }: { bucket: ScoreBucketResult }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950/80">
      <div className="grid grid-cols-[minmax(0,1.1fr)_120px_minmax(0,1.2fr)] gap-4 border-b border-zinc-800 px-4 py-3 text-[11px] uppercase tracking-[0.2em] text-zinc-500">
        <p>Criterion</p>
        <p>Status</p>
        <p>Assessment</p>
      </div>
      {bucket.criteria.map((criterion) => (
        <div
          key={criterion.key}
          className="grid grid-cols-1 gap-3 border-b border-zinc-800 px-4 py-4 last:border-b-0 md:grid-cols-[minmax(0,1.1fr)_120px_minmax(0,1.2fr)] md:gap-4"
        >
          <div>
            ...
            {criterion.evidence.length ? (
              <ul className="mt-2 space-y-2 text-xs leading-5 text-zinc-400">
                {criterion.evidence.map((item) => (
                  <li key={item} className="rounded-xl border border-zinc-800 bg-zinc-900/80 px-3 py-2">
                    {item}
                  </li>
                ))}
              </ul>
    ...
    <section className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-5">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-zinc-400">{bucket.helpText}</p>
        </div>
  ...
  return (
    <main className="min-h-screen bg-zinc-950 px-6 py-10 text-zinc-50">
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        <section className="rounded-[2rem] border border-zinc-800 bg-zinc-900/60 p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              <span className="rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1 text-sm text-zinc-300">{resultOutputProfile.label}</span>
              <Link
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-5">
              ...
              <p className="mt-3 text-sm leading-7 text-zinc-300">
                Full detailed audit view with bucket scores, criterion status, notes, and evidence. The main report stays lightweight, this page is the full inspection layer.
              </p>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {result.structuredAnalysis.buckets.map((bucket) => (
                <div key={bucket.key} className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 px-4 py-3 text-xs leading-6 text-zinc-500">
          Read this like an audit sheet: score at the top, then each criterion with status, explanation, and supporting evidence.
        </div>
